...
public abstract class Designador extends Expresion {
    ...
    public void compilarExpresion() {
        GeneradorCodigo.comentario("Sacamos la dirección del designador");
        compilarDesignador();
        //Tiene que ser básico
        GeneradorCodigo.comentario("Cargamos el valor dado por esa dirección");
    ...
    public void compilarAsignacion() {
        ...
        GeneradorCodigo.comentario("Cambiamos el orden: DST, SRC -> SRC, DST");
        GeneradorCodigo.llamar(GeneradorCodigo.CAMBIAR);

        GeneradorCodigo.comentario(String.format("Copiamos el tamaño del tipo entre 4"));
